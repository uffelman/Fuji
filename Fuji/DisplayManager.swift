...
protocol DisplayManaging: AnyObject {
    ...
    ///
    /// - Parameter displayID: The Core Graphics display ID to reset
    /// - Returns: `true` if the reset was successful, `false` otherwise
    @discardableResult
    func resetToDefault(displayID: CGDirectDisplayID) -> Bool

    /// Returns the display that currently has keyboard focus.
    ///
    /// Uses `NSScreen.main` which reports the screen containing the window that
    /// currently receives keyboard events system-wide. Falls back to the primary
    /// display if no screen can be determined.
    /// - Returns: The active Display, or nil if it cannot be determined
    func activeDisplay() -> Display?
}
...
final class DisplayManager: DisplayManaging {
    ...
    private func createDisplay(for displayID: CGDirectDisplayID) -> Display? {
        ...
        // Fetch raw CG modes once and share across helpers
        let options: CFDictionary =
            [kCGDisplayShowDuplicateLowResolutionModes: kCFBooleanTrue] as CFDictionary
        let cgModes = CGDisplayCopyAllDisplayModes(displayID, options) as? [CGDisplayMode] ?? []

        let modes = getDisplayModes(from: cgModes)
        let currentMode = getCurrentMode(for: displayID)
        let defaultMode = getDefaultMode(for: displayID, cgModes: cgModes, filteredModes: modes)
    ...
    private func getDefaultMode(
        for displayID: CGDirectDisplayID,
        cgModes: [CGDisplayMode],
        filteredModes modes: [DisplayMode]
    ) -> DisplayMode? {
        // Get native panel resolution by analyzing available display modes
        guard let nativePanelResolution = getNativePanelResolution(from: cgModes) else {
            return nil
        }
    ...
    private func getNativePanelResolution(from modesArray: [CGDisplayMode]) -> (
        width: Int, height: Int
    )? {
        guard !modesArray.isEmpty else { return nil }
    ...
    private func getDisplayName(for displayID: CGDirectDisplayID) -> String {
        ...
        if CGDisplayIsBuiltin(displayID) != 0 {
            return String(localized: .displayBuiltIn)
        }

        return String(localized: .displayGeneric(String(displayID)))
    }
    ...
    private func getDisplayModes(from modesArray: [CGDisplayMode]) -> [DisplayMode] {
    ...
    /// Resets a single display to its default resolution.
    ///
    /// Looks up the display's default mode and applies it.
    ///
    /// - Parameter displayID: The Core Graphics display ID to reset
    /// - Returns: `true` if the reset was successful, `false` if the display or its default mode was not found
    func resetToDefault(displayID: CGDirectDisplayID) -> Bool {
        guard let display = displays.first(where: { $0.id == displayID }),
              let defaultMode = display.defaultMode else {
            return false
        }
        return setDisplayMode(defaultMode, for: displayID)
    }
...
nonisolated private func displayReconfigurationCallback(
    ...
    // The callback fires twice per change: once with beginConfigurationFlag (before
    // the change takes effect) and once without it (after). Skip the begin event
    // because display data is still stale at that point.
    guard
        let userInfo,
